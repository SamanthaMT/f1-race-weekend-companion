...
def detect_battles(battle_data):
    ...
    for record in battle_data:
        ...
        int = record.get("interval")

        #May need (isinstance(int, str) and int.endswith("L"))

        if int in [None, "null"] or int.endswith("L") or "lap_number" not in record:
            continue
